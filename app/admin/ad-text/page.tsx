...
export default function AdminAdTextsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/ad-texts`, {
        headers: {
  ...
  const handleAdTextSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && adTextForm.id) {
        url = `${BACKEND_URL}/ad-texts/${adTextForm.id}`;
        method = "PUT";
      } else {
        url = `${BACKEND_URL}/ad-texts`;
        method = "POST";
      }
  ...
  const handleDeleteAdText = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/ad-texts/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
